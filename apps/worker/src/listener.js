...
export async function startListener() {
    ...
    contract.on("ExecutionRequested", async (wallet, project, fn, cid, requestIdHex, event) => {
        const requestId = BigInt(requestIdHex).toString(); 
        console.log(`[Event] ExecutionRequested: ${project}/${fn} (ReqID: ${requestId})`);

        try {
            const GATEWAY_URL = process.env.GATEWAY_URL;
            if (!GATEWAY_URL) {
                console.warn("[Worker] Missing GATEWAY_URL env var");
            }
            console.log(`[Worker] 🚀 Starting parallel fetch: IPFS code + inputs`);
            const startTime = Date.now();

            const [code, inputs] = await Promise.all([
                fetchFunctionCode(cid),
                (async () => {
                    for (let attempt = 1; attempt <= 5; attempt++) {
                        try {
                            ...
                            if (res.ok) {
                                ...
                                const fetchedInputs = data.http?.body || data;
            ...
            console.log(`[Worker] ✅ Parallel fetch completed in ${fetchTime}ms`);
